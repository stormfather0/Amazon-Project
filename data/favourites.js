...
export function addFavourite(productId) {
    ...
    // Add the product to the local storage favorite list
    if (!favourite.includes(productId)) {
        favourite.push(productId);
        localStorage.setItem('favourite', JSON.stringify(favourite));
    }

    // Send a POST request to add the favourite to the backend (MongoDB)
    fetch('https://amazon-project-sta4.onrender.com/api/favourites', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`  // Ensure this token is correct
        },
...
export function removeFavourite(productId) {
    ...
    // Remove the product from local storage favourite list
    ...
    // Send a DELETE request to remove the favourite from the backend (MongoDB)
    fetch('https://amazon-project-sta4.onrender.com/api/favourites', {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
